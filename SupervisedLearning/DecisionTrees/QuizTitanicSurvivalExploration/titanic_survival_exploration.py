...
# Load the dataset
in_file = 'DecisionTrees/QuizTitanicSurvivalExploration/titanic_data.csv'
...
# Now, let's do some data preprocessing. First, we'll remove the names of the passengers, and then one-hot 
# encode the features.
# One-Hot encoding is useful for changing over categorical data into numerical data, with each different 
# option within a category changed into either a 0 or 1 in a separate *new* category as to whether it is that 
# option or not (e.g. Queenstown port or not Queenstown port). Check out [this article](https://hackernoon.com/what-is-one-hot-encoding-why-and-when-do-you-have-to-use-it-e3c6186d008f) before continuing. 
...
model = DecisionTreeClassifier(criterion="gini", max_depth=7, min_samples_leaf=5, min_samples_split=6)
model.fit(X_train, y_train)
